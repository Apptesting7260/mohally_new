...
class EnglishSearchController extends GetxController {
  final AuthRepository _api = AuthRepository();

  final rxRequestStatus = Status.LOADING.obs;
  final products = SearchModel().obs;
  RxString error = ''.obs;
  RxString searchText = ''.obs;
  // final searchtext = TextEditingController().obs;

  void setRxRequestStatus(Status value) => rxRequestStatus.value = value;
  void setProducts(SearchModel value) => products.value = value;
  ...
  void searchProducts(String searchText) async {
    ...
    loading.value = true;
    Map data = {"pro_name": searchText, "language_type": "English"};
    var header = {'Authorization': "Bearer $token"};

    _api.ProductSearchByNameApi(data, header).then((value) {
